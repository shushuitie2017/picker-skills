...
> **★画幅朝向 `--orient`★**：默认 `landscape`（16:9，1920×1080，横版录屏/站内播放）。做**竖屏 9:16 短视频**(抖音/视频号/Shorts/小红书)
> 加 `--orient=portrait` → 产 1080×1920 deck（覆盖 Stage/useStageScale + base.css 的 stage-frame 尺寸；横屏路径字节不变）。
> 章节需按竖屏**纵向堆叠**重排版式。
