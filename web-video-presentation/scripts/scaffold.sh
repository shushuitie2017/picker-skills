...
#
# 用法：
#   bash scripts/scaffold.sh <target-dir> [--theme=<id>] [--orient=landscape|portrait]
...
#   bash <path-to-web-video-presentation>/scripts/scaffold.sh ./presentation-v --theme=guji-ink --orient=portrait
#   bash <path-to-web-video-presentation>/scripts/scaffold.sh --list-themes
#
# --orient（默认 landscape=16:9，1920×1080）：
#   portrait = 9:16 竖屏（1080×1920），用于抖音/视频号/Shorts 等竖屏短视频导出。
#   竖屏只覆盖与画幅尺寸耦合的少数文件（Stage.tsx / useStageScale.ts + base.css
#   末尾追加 .stage-frame 尺寸覆盖），其余共用横屏模板。横屏路径字节级不变。
#
...
if [[ "$ORIENT" != "landscape" && "$ORIENT" != "portrait" ]]; then
  echo "✗ --orient 只能是 landscape 或 portrait（收到 '${ORIENT}'）" >&2
  exit 1
fi
...
echo "▸ 使用主题：$THEME"
echo "▸ 画幅朝向：$ORIENT$([[ "$ORIENT" == portrait ]] && echo '（9:16 竖屏 1080×1920）' || echo '（16:9 横屏 1920×1080）')"
...
# ── 竖屏覆盖（--orient=portrait）──
# 横屏路径到此为止与原来字节级一致；竖屏只覆盖与画幅尺寸耦合的少数文件。
echo "$ORIENT" > .orient
if [[ "$ORIENT" == "portrait" ]]; then
  echo "▸ 应用竖屏覆盖（1080×1920）"
  cp "$TEMPLATES/portrait/Stage.tsx"        src/components/Stage.tsx
  cp "$TEMPLATES/portrait/useStageScale.ts" src/hooks/useStageScale.ts
  # 把 .stage-frame 尺寸覆盖追加到 base.css 末尾（在 theme tokens.css 之后导入 → 必胜）
  printf '\n' >> src/styles/base.css
  cat "$TEMPLATES/portrait/base-portrait.css" >> src/styles/base.css
fi
...
当前主题：${THEME}（见 .theme）
当前朝向：${ORIENT}（见 .orient）$([[ "$ORIENT" == portrait ]] && echo ' —— 竖屏 9:16，1080×1920；章节请按纵向堆叠重排版式')
